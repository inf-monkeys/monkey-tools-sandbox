...
@Injectable()
export class CollectDowhileOutputService {
  ...
  private async collectDoWhileInputInSubworkflow(
    workflowInstanceId: string,
    collectData: (taskReferenceName: string, data: any) => void,
  ) {
    const data = await this.conductorClient.workflowResource.getExecutionStatus(
      workflowInstanceId,
      true,
    );
    const { tasks } = data;
    for (const task of tasks) {
      ...
      if (taskType === 'SUB_WORKFLOW') {
        await this.collectDoWhileInputInSubworkflow(
          outputData.subWorkflowId,
          collectData,
        );
      } else {
  ...
  public async collectDowhileOutput(
    ...
    const { tasks } = data;
    const doWhileTask = tasks.find(
      (t: any) =>
        t.taskType === 'DO_WHILE' &&
        t.referenceTaskName === doWhileTaskReferenceName,
    );
    if (!doWhileTask) {
